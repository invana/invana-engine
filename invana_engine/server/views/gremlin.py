...
class GremlinQueryView(WebSocketEndpoint):
    ...
    async def on_connect(self, websocket):
        ...
        await self.gremlin.connect({GREMLIN_SERVER_SETTINGS['gremlin_server_url']})
    ...
    async def prepare_message(gremlin_query):
        ...
        query_message = {
            ...
            "args": {
                ...
                "aliases": {"g": {GREMLIN_SERVER_SETTINGS['traversal_source']}},
                "session": request_id
            }
        }
        if {GREMLIN_SERVER_SETTINGS['gremlin_server_username']} or {GREMLIN_SERVER_SETTINGS['gremlin_server_password']}:
            auth = b"".join([b"\x00", {GREMLIN_SERVER_SETTINGS['gremlin_server_username']}.encode("utf-8"),
                             b"\x00", {GREMLIN_SERVER_SETTINGS['gremlin_server_password']}.encode("utf-8")])
