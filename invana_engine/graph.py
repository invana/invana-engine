    ...
    def __init__(self):
        ...
        if self.backend_cls_str == "CypherConnector":
            self.backend = backend_cls(
                getattr(settings, "GRAPH_BACKEND_URL"),
            ...
            self.backend = backend_cls(
                getattr(
                    settings, "GRAPH_BACKEND_URL"), database_name=getattr(
                    settings, "GRAPH_BACKEND_DATABASE_NAME"), username=getattr(
                    settings, "GRAPH_BACKEND_AUTH_USERNAME"), password=getattr(
                    settings, "GRAPH_BACKEND_AUTH_PASSWORD"), traversal_source=getattr(
                        settings, "GRAPH_BACKEND_GREMLIN_TRAVERSAL_SOURCE"))
