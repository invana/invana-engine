...
class VertexOperations(CRUDOperationsBase):
    ...
    def _read_many(self, label=None, query=None, limit=10, skip=0):
        logger.debug("Reading vertices with label:{label};  query: {query}; limit: {limit}"
                     "; skip: {skip}".format(label=label, query=query, limit=limit, skip=skip))
    ...
    #     return cleaned_data
    #
    # def filter_vertex_and_neighbor_edges_and_vertices(self, vertex_id=None, label=None, namespace=None, query=None,
